...
def youtube(url: str) -> str:
    ...
    duration = round(info["ماوە"] / 60)
    try:
        ydl.download([url])
    except Exception as e:
        print(e)
        pass
    return path.join("داگیراوەکان", f"{info['id']}.{info['ext']}")

RADIO_TITLE=os.environ.get("RADIO_TITLE", "🎸 گۆرانی 24/7 | مۆدی ڕادیۆ")
if RADIO_TITLE=="NO":
    RADIO_TITLE = None
...
class MusicPlayer(object):
    ...
    async def send_playlist(self):
        if not playlist:
            pl = f"{emoji.NO_ENTRY} لیستی پەخشی بەتاڵ"
        else:       
            pl = f"{emoji.PLAY_BUTTON} **لیستی پەخشکردن**:\n" + "\n".join([
                f"**{i}**. **🎸{x[1]}**\n   👤**داواکراوە لەلایەن:** {x[4]}\n"
                for i, x in enumerate(playlist)
            ])
        if msg.get('لیستی پەخشکردن') is not None:
            await msg['لیستی پەخشکردن'].delete()
        msg['لیستی پەخشکردن'] = await self.send_text(pl)
    async def skip_current_playing(self):
        ...
        print(f"- دەست بکە بە پەخشکردنی گۆرانی : {playlist[0][1]}")
        if LOG_GROUP:
    ...
    async def delete(self, message):
        if message.chat.type == "سوپەرگروپ":
            await sleep(DELAY)
            try:
    ...
    async def get_admins(self, chat):
        ...
        if not admins:
            ...
            try:
                grpadmins=await bot.get_chat_members(chat_id=chat, filter="بەڕێوەبەرەکان")
